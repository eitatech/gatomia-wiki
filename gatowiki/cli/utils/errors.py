...
class GatoWikiError(Exception):
    """Base exception for GatoWiki CLI errors."""
...
class ConfigurationError(GatoWikiError):
...
class RepositoryError(GatoWikiError):
...
class FileSystemError(GatoWikiError):
...
def handle_error(error: Exception, verbose: bool = False) -> int:
    ...
    """
    if isinstance(error, GatoWikiError):
